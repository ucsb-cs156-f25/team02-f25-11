...
describe("RecommendationRequestForm tests", () => {
  ...
  test("email max length prevents submit", async () => {
    ...
    expect(submitSpy).not.toHaveBeenCalled();
    expect(
      await screen.findByText(/Max length 254 characters/),
    ).toBeInTheDocument();
  });
  ...
  test("explanation max length shows correct error message", async () => {
    ...
    expect(
      await screen.findByText(/Max length 2000 characters/),
    ).toBeInTheDocument();
  });
